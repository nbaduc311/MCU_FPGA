...
{
	// Chỉ cần xóa cờ, không thực hiện đếm gì nữa
...
void CT16B0_Init(void)
{
	...
	SN_CT16B0->MR9 = 12000000 / 4000 - 1;	// HCLK=12MHz. Tần số chu kỳ 4000Hz cho Còi
	
	SN_CT16B0->MCTRL = 1<<30;			// when TC == MR9,reset TC = 0; (Chỉ giữ Reset TC, BỎ luôn ngắt Interrupt MR9 Interrupt ở Bit 29)
	
	SN_CT16B0->TMRCTRL = 1 << 1;		//reset timer count
	while(SN_CT16B0->TMRCTRL & (1<<1));
	
	// Kích hoạt xuất tín hiệu PWM0 ra chân GPIO (Còi)
